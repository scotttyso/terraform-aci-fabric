/*
SNMP Trap Source Group and Destination Group
API Information:
 - Class: "snmpSrc"
...
resource "aci_rest" "snmp_trap_src" {
	path       = "/api/node/mo/uni/fabric/moncommon/snmpsrc-${var.snmp_src}.json"
	class_name = "snmpSrc"
	payload    = <<EOF
{
	"snmpSrc": {
		"attributes": {
			"dn": "uni/fabric/moncommon/snmpsrc-${var.snmp_src}",
			"incl": "${var.messages}",
			"name": "${var.snmp_src}",
		},
		"children": [
			{
				"snmpRsDestGroup": {
					"attributes": {
						"tDn": "uni/fabric/snmpgroup-${var.dest_grp}",
					},
					"children": []
				}
			}
